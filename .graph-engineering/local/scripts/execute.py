...
def validate_inputs(inputs: CommandInputs) -> None:
    if inputs.execution_type == "new":
        ...
        if not project_root.is_dir() or not (project_root / ".codex").is_dir():
            fail("--project-root must identify a project directory containing .codex.")
...
def resolve_script_command(script_command: str, snapshot: Snapshot) -> str:
    """Interpolate context fields and `{project_root}` from the snapshot."""
    ...
    if "{project_root}" in interpolated_command:
        project_root = snapshot.get("project_root")
        if not isinstance(project_root, str) or not project_root:
            fail("Script reference '{project_root}' does not exist in the snapshot.")
        interpolated_command = interpolated_command.replace(
            "{project_root}", project_root
        )
    if "{" in interpolated_command or "}" in interpolated_command:
        fail(
            "Invalid script interpolation. Only '{project_root}' and "
...
def execute_script(
    ...
    for script_command in current_state.get("scripts", []):
        ...
        script_identifier = command_parts[0]
        ...
        result = subprocess.run(
            [python_exe, str(resolved_script.script_path)] + arguments,
